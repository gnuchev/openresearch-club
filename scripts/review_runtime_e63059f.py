"""Focused W2/W4/W7 follow-up probes on the isolated review Worker only."""
import argparse
import hashlib
import http.client
import json
...
def main():
    ...
    env = dict(os.environ, XDG_CONFIG_HOME=str(review/'xdg'), WRANGLER_LOG_PATH=str(review/'e63059f-edge-tools.log'), WRANGLER_SEND_METRICS='false')
    checks = []
    ...
    try:
        run = subprocess.run(['node',str(ROOT/'node_modules/wrangler/bin/wrangler.js'),'d1','execute','openresearch-club','--local','--config',str(config),'--persist-to',str(review/'state-e63059f'),'--file',str(fixture)],cwd=ROOT,env=env,capture_output=True,text=True,encoding='utf-8',errors='replace',timeout=60,creationflags=0x08000000 if os.name=='nt' else 0)
    ...
    # Verify stream limits without Content-Length, using small bounded fixtures.
    def chunked(path,ctype,chunks):
        conn=http.client.HTTPConnection('127.0.0.1',8791,timeout=20)
        try:
            conn.request('POST' if ctype=='application/json' else 'PUT',path,body=iter(chunks),headers={'Authorization':'Bearer '+author['token'],'Content-Type':ctype,'Idempotency-Key':'edge-stream-'+secrets.token_hex(8)},encode_chunked=True)
            res=conn.getresponse();data=res.read()
            return res.status,data.decode(errors='replace')
        finally:conn.close()
    status,_=chunked('/v1/posts','application/json',[b'{"body_md":"',b'x'*(600*1024),b'x'*(600*1024),b'"}'])
    record('chunked_json_over_one_mib_refused',413,status)
    status,_=chunked('/v1/artifacts/'+uid()+'/content','application/octet-stream',[b'x'])
    record('binary_upload_without_content_length_refused',411,status)

    report=dict(reviewed_commit=subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),source_tree=subprocess.check_output(['git','rev-parse','HEAD:src'],cwd=ROOT,text=True).strip(),scope='Local-only HTTP edge cases for W2, W4 and W7, on the isolated state-e63059f database; synthetic credentials remain in memory.',checks=checks,passed=sum(c['passed'] for c in checks),failed=sum(not c['passed'] for c in checks))
