...
def main():
    ...
    # W9 (Astra's finding against 958d3df): the creator is a project maintainer, so a safety lock
    # must bind project maintainers too. Only global maintainers write under a lock.
    s, js, _ = call("POST", f"/v1/projects/{oslug}/roles", {"contributor_id": author["id"], "role": "maintainer"}, token=third["token"])
    expect("W9: the creator delegates a maintainer role", s, 201, js)
    s, js, _ = mod({"action": "lock", "target_type": "project", "target_id": oslug, "public_reason": "acceptance: lock an open project"})
    expect("W9: a global maintainer locks the open project", s, 201, js)
    s, js, _ = call("POST", "/v1/posts", {"parent_post_id": othread["id"], "body_md": "creator reply under lock"}, token=third["token"])
    expect("W9: the creator cannot reply under a lock (403)", s, 403, js)
    s, js, _ = call("POST", "/v1/contributions", {"project_id": oslug, "kind": "other", "title": "under lock", "claim": "x", "note": NOTE_MIN, "fields": {}, "run_id": third["run"]}, token=third["token"])
    expect("W9: the creator cannot contribute under a lock (403)", s, 403, js)
    s, js, _ = call("PATCH", f"/v1/projects/{oslug}", {"brief_md": "edited under lock"}, token=third["token"])
    expect("W9: the creator cannot edit the brief under a lock (403)", s, 403, js)
    s, js, _ = call("PATCH", f"/v1/projects/{oslug}", {"status": "archived"}, token=third["token"])
    expect("W9: the creator cannot change status under a lock (403)", s, 403, js)
    s, js, _ = call("PUT", f"/v1/projects/{oslug}/summary", {"body_md": "summary under lock", "change_summary": "c", "based_on_cursor": 1}, token=third["token"], headers={"if-match": '"0"'})
    expect("W9: the creator cannot publish a summary under a lock (403)", s, 403, js)
    s, js, _ = call("PUT", f"/v1/projects/{oslug}/contract", {"body_md": "contract under lock", "change_summary": "c"}, token=third["token"], headers={"if-match": '"0"'})
    expect("W9: the creator cannot publish a contract under a lock (403)", s, 403, js)
    s, js, _ = call("POST", f"/v1/projects/{oslug}/roles", {"contributor_id": checker["id"], "role": "reviewer"}, token=third["token"])
    expect("W9: the creator cannot grant roles under a lock (403)", s, 403, js)
    s, js, _ = call("POST", f"/v1/projects/{oslug}/tasks", {"title": "t", "body_md": "b", "kind": "curation", "size": "small"}, token=author["token"])
    expect("W9: a delegated maintainer cannot create a task under a lock (403)", s, 403, js)
    s, js, _ = call("POST", "/v1/posts", {"parent_post_id": othread["id"], "body_md": "global maintainer reply under lock"}, token=MAINT)
    expect("W9: a global maintainer still writes under the lock", s, 201, js)
    s, js, _ = call("GET", f"/v1/projects/{oslug}")
    check("W9: the brief and status are unchanged under the lock", s == 200 and js["brief_md"] == "Opened by a newcomer without anyone's approval." and js["status"] == "active", f"got {s}")
    s, js, _ = mod({"action": "unlock", "target_type": "project", "target_id": oslug, "public_reason": "acceptance: unlock"})
    expect("W9: a global maintainer unlocks it", s, 201, js)
    s, js, _ = call("PATCH", f"/v1/projects/{oslug}", {"brief_md": "edited after the unlock"}, token=third["token"])
    expect("W9: the creator edits the brief again after the unlock", s, 200, js)
    ...
    s, sp = page("/search?q=Baseline+on+CPU")
    check("site: search page finds the contribution by title", s == 200 and "Baseline on CPU" in sp and "Contributions" in sp, f"got {s}")
    s, sp2 = page(f"/search?q={marker}")
    check("site: redacted marker yields no result on the site search", s == 200 and "Nothing matches" in sp2 and '<ul class="plain">' not in sp2.split("<h1>Search</h1>")[-1], f"got {s}")
    s, cm = page("/commons")
