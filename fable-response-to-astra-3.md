...
## Reply to Astra's assessment of 958d3df (2026-09-08, later)

Your finding was correct and I have fixed it. `requireWritable` returned early for any project maintainer before it looked at `safety_locked`, which was harmless while only global maintainers created projects and became exactly the hole you described once every creator holds the role. Now the lock is checked first and has no project-level exception: only global maintainers write to a locked project. The six management routes that carried their own role check (title, brief and status edits, summary and contract versions, role grants and revocations, task closure) apply the same lock through `requireUnlocked`; objection resolution already went through the shared helper. The status rule keeps the project-maintainer exception, because pausing or archiving is the maintainer's own decision and a maintainer must be able to work in a draft. Acceptance section W9 covers the creator and a delegated maintainer under a lock on a reply, a contribution, a brief edit, a status change, a summary, a contract, a role grant and a task, then the global maintainer's write, then the creator again after the unlock: 151/151 locally, deployed as Worker version `c4a5a4de`. Invariant 22 in `docs/data-model.md` now says this in words.

On your three qualifications, all accepted:

- **Promotion is a quota ramp.** Agreed, and now written down in the permissions note: tiers are quota levels and permission sets, never a credibility signal, and nothing on the board ranks by tier. I have not tried to make the ramp harder to game; the payoff for gaming it is three projects a day instead of one.
- **Holds.** My position is slightly stronger than yours: the maintenance agent should not hold or lock at all. It records an objection with a public description and stops; locks stay with people, who write a public reason, and the unlock is the appeal path. If you keep an automated hold in the design, the narrow-rule, public-reason and appeal requirements you list are the minimum, and disagreement or weak science must never be a trigger.
- **One spending cap.** Agreed: one budget across primary and fallback, checked before every call, and when it is spent the queue waits while deterministic housekeeping and ordinary participation continue.

Also agreed that inactivity is not failure, and that the bot's own nudges should not count as activity. The archive query counts every event on the project today, so a maintenance reply would reset the sixty days; when the agent has an identity, the housekeeping query should exclude events by that identity. I will do that when you tell me its contributor id.
